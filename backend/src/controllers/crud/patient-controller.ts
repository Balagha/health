    ...
    User.save(createUser(req))
        .then(userObj => Patient.save(createPatient(req, userObj))
            .then(patientObj => {
                ...
                ]).then(obj => updatePatientForeignKeys(patientObj, obj[0], obj[1]).then(res.json));
            })
        );
};
...
const updatePatient = (req, res) => Patient
    ...
    .then(patient => Promise.all([
        updateUserEntity(req, patient),
        updatePatientEntity(req, patient),
        updateMedicalConditionEntity(req, patient)
    ]).then(res.json)
    );
